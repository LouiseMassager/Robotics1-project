...
class SimpleSubscriber(Node):
    ...
    def __init__(self):
        ...
        # angles
        self.pub = self.create_publisher(Twist, 'cmd_thetas', 10)
        self.msg_thetas = Twist()
    ...
    def listener_callback(self, msg):
        ...
        res = [0., 1.571, 0., 0., 0., 0.]
        self.get_logger().info('I computed: "%s"' % str(res))
        self.msg_thetas.linear.x = res[0]
        self.msg_thetas.linear.y = res[1]
        self.msg_thetas.linear.z = res[2]
        self.msg_thetas.angular.x = res[3]
        self.msg_thetas.angular.y = res[4]
        self.msg_thetas.angular.z = res[5]
        self.pub.publish(self.msg_thetas)
...
def ik(Ox, Oy, Oz, psi, theta, phi):
    # inputs (in radiants):
    #    psi = 0  # psi=float(sys.argv[1])
    #    theta = 0  # theta=float(sys.argv[2])
    #    phi = 0  # phi=float(sys.argv[3])
    #    Ox = 0
    #    Oy = 0
    #    Oz = l1 + l2 + l3 + l4 + l5 + l6  # end-effector position
    # with  strainght position => Oz = l1 + l2 + l3 + l4 + l5 + l6
    # p=1: return [0°,0°,0°,90°,90°,90°]
    # p=2: return [0°,0°,0°,90°,-90°,90°]
    # p=3: return [180°,0°,0°,90°,90°,-90°]
    # p=3: return [180°,0°,0°,90°,-90°,-90°]
